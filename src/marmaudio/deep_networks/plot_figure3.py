...
def plot_points_with_labels(df):
    ...
    for c, grp in df.groupby('label'):
        if c == 'Vocalization':
            ax.scatter(grp.umap_x, grp.umap_y, s=1, alpha=.025, c='grey')
        else:
            scatter = ax.scatter(grp.umap_x, grp.umap_y, s=4, alpha=.1, c=None)
    ...
    plt.savefig('projection_points_with_labels_with_legend.png', bbox_inches='tight', dpi=300)
...
if __name__ == "__main__":
    encodings = np.load('encodings_AE_marmosset_logMel128_16feat_all-vocs._10-05-25.npy', allow_pickle=True).item()
    idxs, umap = encodings['idx'], encodings['umap']
    tsv_file = f"/media/charly/9A0CEE3F0CEE1653/Charly/Annotations.tsv"
    df = pd.read_csv(tsv_file, sep='\t')
    idxs = [os.path.basename(idx) for idx in idxs]

    """encodings = np.load('encodings_segmented_AE_macaque_logMel128_16feat.npy', allow_pickle=True).item()
    idxs, umap = encodings['idx'], encodings['umap']
    df = pd.read_csv('macaque_voc_vs_noise_segmented_filtered_predictions_dynamic_2023-05-13_all.tsv', sep='\t')"""
