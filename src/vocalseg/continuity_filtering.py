...
def continuity_segmentation(
    ...
    interim_plot_number = 0

    def plot_interim(spec, cmap=plt.cm.afmhot, zero_nan=False, interim_plot_number=0):
        print(f"[DEBUG] Interim plot '{interim_plot_id}_{interim_plot_number}.png'...")
        ...
        plt.savefig(f'{interim_plot_id}_{interim_plot_number}.png')
        interim_plot_number += 1
        plt.close()
        return interim_plot_number
    ...
    if results is None:
        if verbose:
            print('dynamic_threshold_segmentation() returned none')
        return None
    ...
    if verbose:
        interim_plot_number += plot_interim(spec, cmap=plt.cm.Greys)

    ### create a mask
    mask = mask_spectrogram(spec, mask_thresh_std)

    if verbose:
        interim_plot_number += plot_interim(mask)
    ...
    if verbose:
        interim_plot_number += plot_interim(elements, cmap=cmap, zero_nan=True)
